...
impl<'a> Runtime<'a> {
    ...
    fn abort(&self, args: &RuntimeArgs) -> Result<()> {
        ...
        println!("msg_ptr: {:?} filename_ptr: {:?} line: {:?} col: {:?}", msg_ptr, filename_ptr, line, col);

        let msg = self.read_buffer_with_size(msg_ptr, 64)?;
        let filename = self.read_buffer_with_size(filename_ptr, 64)?;

        println!("msg: {:?} filename: {:?} line: {:?} col: {:?}", msg, filename, line, col);
